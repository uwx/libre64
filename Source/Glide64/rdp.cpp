...
bool CRDP::init()
{
    ...
    m_frame_buffers = new COLOR_IMAGE[NUMTEXBUF + 2];
    if (m_frame_buffers == NULL)
    {
        free();
        return false;
    }
...
static void CopyFrameBuffer(gfxBuffer_t buffer = GFX_BUFFER_BACKBUFFER)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (gfxLfbLock(GFX_LFB_READ_ONLY, buffer, GFX_LFBWRITEMODE_565, GFX_ORIGIN_UPPER_LEFT, false, &info))
            {
        ...
        */
        ...
        case ci_zimg:
            if (g_settings->ucode() != CSettings::ucode_PerfectDark)
            {
                ...
                {
                    gfxLOD_t LOD = g_settings->scr_res_x() > 1024 ? GFX_LOD_LOG2_1024 : GFX_LOD_LOG2_2048;
