...
void CSupportWindow::Show(HWND hParent)
{
    ...
    if (m_RunCount == -1 || ValidPatreonEmail())
    {
        return;
    }
...
LRESULT CSupportWindow::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
...

    {
        ...
        HDC hDC = hInfo.GetDC();
        HFONT hFont = (HFONT)hInfo.SendMessage(WM_GETFONT);
        if (hFont == NULL)
        {
        ...
        {
            hInfo.SetWindowPos(NULL, 0, 0, rcWin.right - rcWin.left, rcWin.bottom - rcWin.top, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOOWNERZORDER);
        }
        SetWindowTextW(hInfo, InfoText.c_str());

        CWindow hEnterPatreon = GetDlgItem(IDC_ENTER_PATREON);
        hInfo.GetWindowRect(&rcWin);
        ::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rcWin, 2);
        GetDlgItem(IDC_ENTER_PATREON).SetWindowPos( NULL, rcWin.left, rcWin.bottom + 4, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOOWNERZORDER);
        m_EnterPatreon.SubclassWindow(hEnterPatreon);
        m_EnterPatreon.SetHyperLinkExtendedStyle(HLINK_COMMANDBUTTON, HLINK_COMMANDBUTTON);
        hEnterPatreon.GetWindowRect(&rcWin);
        ::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rcWin, 2);
        CWindow hInfoNotification = GetDlgItem(IDC_INFO_NOTIFICATION);
        hInfoNotification.SetWindowPos(NULL, rcWin.left, rcWin.bottom + 8, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOOWNERZORDER);
        std::wstring EnterCodeInfoText = wGS(MSG_SUPPORT_ENTER_CODE_INFO);
        if (DrawTextW(hDC, EnterCodeInfoText.c_str(), EnterCodeInfoText.length(), &rcWin, DT_LEFT | DT_CALCRECT | DT_WORDBREAK | DT_NOCLIP) > 0)
        {
            hInfoNotification.SetWindowPos(NULL, 0, 0, rcWin.right - rcWin.left, rcWin.bottom - rcWin.top, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOOWNERZORDER);
        }
        SetWindowTextW(hInfoNotification, EnterCodeInfoText.c_str());
        hInfoNotification.GetWindowRect(&rcWin);
        ::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rcWin, 2);

        CWindow EnterCode = GetDlgItem(IDC_ENTER_CODE);
        EnterCode.SetWindowPos(NULL, rcWin.left, rcWin.bottom + 8, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOOWNERZORDER);
...
LRESULT CSupportWindow::OnEnterPatreonEmail(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ChangePatreonEmail(m_hWnd);
    if (ValidPatreonEmail())
    {
        EndDialog(wID);
    }
    return TRUE;
